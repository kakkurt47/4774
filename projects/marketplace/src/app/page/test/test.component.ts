...
import {MuzikaCoin, MuzikaPaperContract, IMuzikaCoin, IMuzikaPaperContract, TruffleContract} from 'muzika-coin';
        ...
        return new Observable<any>(observer => {
          this.web3.eth.getBalance(acc, (err, balance) => {
            ...
            this.muzikaCoin.deployed().then(ins => {
              ...
              observer.next({
                address: acc,
                balance: this.web3.fromWei(balance).toString(),
                token: tokenBalance.toNumber() / 1e+18
              });
              observer.complete();
            });
          });
