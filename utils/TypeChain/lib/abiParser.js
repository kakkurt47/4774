...
function checkForOverloads(constants, constantFunctions, functions, name) {
    return (constantFunctions.find(function (f) { return f.name === name; }) ||
        constants.find(function (f) { return f.name === name; }) ||
        functions.find(function (f) { return f.name === name; }));
}
function parseOutputs(outputs) {
    if (outputs.length === 0) {
        ...
        return outputs.map(function (param) { return typeParser_1.parseEvmType(param.type); });
    }
}
