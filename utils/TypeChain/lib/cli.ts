...
async function main() {
  ...
  matches.forEach(p => {
    ...
    if (contractName) {
      ...
      importString.push(`import { ${contractName}, createTruffle${contractName} } from './interface/${contractName}';`);
    }
...
${contractNames.map(name => `export { ${name}, I${name}, createTruffle${name} } from './interface/${name}'`).join(';\n')}

${contractNames.map(name => `
export const ${name}ProviderFactory = (web3: ExtendedWeb3, platformId: string) => {
  if (isPlatformBrowser(platformId)) {
    const contract: ${name} = createTruffle${name}();
...
${contractNames.map(name => `  { provide: ${name}, useFactory: ${name}ProviderFactory, deps: [ExtendedWeb3, PLATFORM_ID] }`).join(',\n')}
];
`;
